/**
 * THE suggestion engine — single source of truth.
 *
 * Imported by BOTH the admin app (src/lib/suggestion.ts shim) and the
 * missionary-sheet pull (api/_lib/suggestion.ts shim). These used to be two
 * hand-maintained copies that had already drifted (the api copy dropped
 * filtered-reasons and shortened hints), which meant the app and the sheet
 * were on track to rank members differently after the next tuning change.
 *
 * Keep this file dependency-free (no Database types, no src/ or api/ imports)
 * so both bundlers can consume it.
 */
...
export type SuggestionInput = {
  ...
  recentOutings: SuggestionOuting[]
...
export function suggest(input: SuggestionInput): SuggestionResult {
  ...
  // Hints render in BOTH UIs (AdminSuggest and the sheet's "Why #1" cell) —
  // keep the wording medium-neutral (no "slots below", which only exists in
  // the app).
  ...
  if (limited.length === 0) {
    if (availableSlots.length === 0) {
      ...
      hint =
        'No Spanish-speaking members are available at that day/slot. Try a different day or time — or ask the WML to recruit more Spanish-speaking members.'
    } else {
      hint = 'No one is available for that day and slot. Try a different day or time.'
    }
